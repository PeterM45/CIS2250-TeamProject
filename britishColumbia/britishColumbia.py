...
def main(argv, fn, index):
  ...
  for year in range(start_year, end_year):
    sortedFrequencyPeople = sorted(people,
                                   key=lambda x: sortFrequency(x, year))
  ...
  with open(fn, 'w', newline='') as file:
    ...
    for year in range(start_year, end_year):
      temp = -1
      new_rank = 0
      rankedList = [(person.name, person.getRank(year)) for person in people
...
if __name__ == "__main__":
  ...
  main(sys.argv, "britishColumbiaMales.csv", 1)
  main(sys.argv, "britishColumbiaFemales.csv", 2)