...
# Output file names
boys_output_file = 'newfoundlandAndLabradorMales.csv'
girls_output_file = 'newfoundlandAndLabradorFemales.csv'
...
for filename in os.listdir(source_dir):
    ...
    if filename.startswith('Top100BabyNames') and filename.endswith('.csv'):
        ...
        if first_line == 'BoysName,Rank,GirlsName':
            ...
            boys_data = pd.concat([
                boys_data, source_data[['Rank', 'BoysName']].rename(columns={
                    'BoysName': 'Name'
                }).assign(Frequency='-1', Year=year)
            ])
            girls_data = pd.concat([
                girls_data, source_data[['Rank', 'GirlsName']].rename(columns={
                    'GirlsName': 'Name'
                }).assign(Frequency='-1', Year=year)
            ])
        #Else, if file has the other header type:
            ...
            boys_data = pd.concat([
                boys_data, source_data[['BoysRank', 'BoysName']].rename(columns={
                    'BoysRank': 'Rank',
                    'BoysName': 'Name'
                }).assign(Frequency='-1', Year=year)
            ])
            girls_data = pd.concat([
                girls_data, source_data[['GirlsRank', 'GirlsName']].rename(columns={
                    'GirlsRank': 'Rank',
                    'GirlsName': 'Name'
                }).assign(Frequency='-1', Year=year)
            ])
