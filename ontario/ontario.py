...
def main (argv):
    ...
    with open ( "ontario_top_baby_names_female_1917-2019_en_fr.csv" ) as csvDataFile:
        next ( csvDataFile ) 
    ...
    with open ( "ontario_top_baby_names_male_1917-2019_en_fr.csv" ) as csvDataFile:
        next ( csvDataFile ) 
        csvReader = csv.reader(csvDataFile, delimiter=',')
        for row in csvReader:
        ...
        females_df.to_csv("ontarioFemales.csv", sep=',', index=False, encoding='utf-8')

        males = {'Rank':rankM,'Year':yearM,'Name':nameM,'Frequency':frequencyM}
        males_df = pd.DataFrame(males)
        males_df.to_csv("ontarioMales.csv", sep=',', index=False, encoding='utf-8')
