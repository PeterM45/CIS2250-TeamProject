...
def main (argv):
    ...
    with open ( "yob1880-2021.txt" ) as csvDataFile:
        next ( csvDataFile ) 
        csvReader = csv.reader(csvDataFile, delimiter=',')
        for row in csvReader:
            if row[1] == 'F':
                if previousGender == 'M': #The file changes years when it shows males again
        ...
        females_df.to_csv("unitedStatesFemales.csv", sep=',', index=False, encoding='utf-8')

        males = {'Rank':rankM,'Year':yearM,'Name':nameM,'Frequency':frequencyM}
        males_df = pd.DataFrame(males)
        males_df.to_csv("unitedStatesMales.csv", sep=',', index=False, encoding='utf-8')
