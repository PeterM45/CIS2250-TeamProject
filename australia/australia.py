...
df = pd.read_csv('popular_baby_names_1952_to_2021.csv')
...
boys_df[['Rank', 'Year', 'Name', 'Count']].rename(columns={
  'Count': 'Frequency'
}).to_csv('australiaMales.csv', index=False, lineterminator='\n')
girls_df[['Rank', 'Year', 'Name', 'Count']].rename(columns={
  'Count': 'Frequency'
}).to_csv('australiaFemales.csv', index=False, lineterminator='\n')
