...
def main (argv):
    ...
    with open ( "saskatchwanTop20BabyNames.csv" ) as csvDataFile:
        next ( csvDataFile ) 
        ...
        females_df.to_csv("saskatchewanFemales.csv", sep=',', index=False, encoding='utf-8')

        males = {'Rank':rankM,'Year':yearM,'Name':nameM,'Frequency':frequencyM}
        males_df = pd.DataFrame(males)
        males_df.to_csv("saskatchewanMales.csv", sep=',', index=False, encoding='utf-8')
