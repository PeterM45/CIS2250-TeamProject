...
def main (argv):
    ...
    with open ( "20221107_topbabynames.csv" ) as csvDataFile:
        next ( csvDataFile ) 
        ...
        females_df.to_csv("californiaFemales.csv", sep=',', index=False, encoding='utf-8')

        males = {'Rank':rankM,'Year':yearM,'Name':nameM,'Frequency':frequencyM}
        males_df = pd.DataFrame(males)
        males_df.to_csv("californiaMales.csv", sep=',', index=False, encoding='utf-8')
