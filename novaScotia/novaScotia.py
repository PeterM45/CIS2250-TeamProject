...
def main(argv):
    ...
    with open(
        "./novaScotia/NS_Top_Twenty_Baby_Names_-_1920-Current.csv"
    ) as csvDataFile:
        next(csvDataFile)
        ...
        for row in csvReader:
            if row[1] == "F":
                ...
                nameF.append(
                    row[2].title()
                )  # .title() capitalizes the names appropriately
        ...
        females_df.to_csv(
            "./novaScotia/novaScotiaFemales.csv", sep=",", index=False, encoding="utf-8"
        )

        males = {"Rank": rankM, "Year": yearM, "Name": nameM, "Frequency": frequencyM}
        males_df = pd.DataFrame(males)
        males_df.to_csv(
            "./novaScotia/novaScotiaMales.csv", sep=",", index=False, encoding="utf-8"
        )
